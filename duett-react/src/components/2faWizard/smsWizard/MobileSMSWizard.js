...
const SMS2FAWizardMobile = ({ open, setWizard }) => {
  ...
  const handleClose = () => {
    localStorage.setItem('smswizard', 'done');
    setWizard(false);
  };
      ...
      <div className={classes.paper}>
        ...
        >
          ...
          <Button
            variant="contained"
            color={
              activeStep === wizardSteps.length - 1 ? 'primary' : 'primary'
            }
            disableElevation
            style={{ marginBottom: theme.spacing(2) }}
            onClick={handleNext}
          >
            {activeStep === wizardSteps.length - 1 ? 'Got it' : 'Next'}
          </Button>
        </Box>
      </div>
    </Modal>
...
export default SMS2FAWizardMobile;
